...
def multi_head_pooling(keys,
    ...
    def dot_product_pooling(k, v, attn_bias, dropout_rate):
        ...
        if attn_bias:
            ...
            attn_bias_sliced = fluid.layers.slice(
                attn_bias, axes=[2], starts=[0], ends=[1])
            # (batch_size, n_head, key_len)
            product += layers.squeeze(attn_bias_sliced, axes=[2])
        ...
        # (batch_size, n_head, key_len, dim_per_head)
        pooling_out = layers.elementwise_mul(x=v, y=weights, axis=0)
        # (batch_size, n_head, dim_per_head)
        pooling_out = layers.reduce_sum(pooling_out, dim=[2])
    ...
    # (batch_size, n_head, dim_per_head)
    ctx_multiheads = dot_product_pooling(k, v, attn_bias, dropout_rate)
    # (batch_size, emb_dim)
    out = layers.reshape(ctx_multiheads, shape=[0, d_model])
...
def multi_head_structure_attention(queries,
    ...
    def graph_scaled_dot_product_attention(q, k, v, attn_bias, d_key, dropout_rate,
        ...
        :return:
        """
        scaled_q = layers.scale(
            x=q, scale=d_key ** -0.5)  # (batch_size, n_head, n_block, dim_per_head)
        ...
        # [batch, n_heads, n_block, dim_per_head]
        graph_out = layers.matmul(weights, v)
        return graph_out
...
def multi_head_hierarchical_attention(queries,
    ...
    def __compute_graph_bias(q, graph_attn_mask, pos_win):
        ...
        batch_ind = layers.range(0, layers.cast(
            batch_size, dtype='int64'), 1, 'int64')
        # print("batch_ind.shape = %s" % str(batch_ind.shape))
        batch_ind = layers.unsqueeze(
            batch_ind, axes=[1, 2, 3])  # (batch_size, 1, 1, 1)
        ...
        head_ind = layers.unsqueeze(
            head_ind, axes=[0, 2, 3])  # (1, n_heads, 1, 1)
        head_ind = layers.expand(head_ind, expand_times=[
                                 batch_size, 1, query_len, 1])
        # print("head_ind.shape = %s" % str(head_ind.shape))

        query_ind = layers.range(0, layers.cast(
            query_len, dtype='int64'), 1, 'int64')
        # print("query_ind.shape = %s" % str(query_ind.shape))
        query_ind = layers.unsqueeze(
            query_ind, axes=[0, 1, 3])  # (1, 1, query_len, 1)
        query_ind = layers.expand(query_ind, expand_times=[
                                  batch_size, n_head, 1, 1])
        # print("query_ind.shape = %s" % str(query_ind.shape))
        ...
        graph_attn_mask = layers.expand(
            graph_attn_mask, expand_times=[1, 1, query_len, 1, 1])
        # print("graph_attn_mask.shape = %s" % str(graph_attn_mask.shape))

        # (batch_size, n_heads, query_len, key_s_len)
        graph_attn_mask_up = layers.gather_nd(
            input=graph_attn_mask, index=pos_up_ind)
        graph_attn_mask_down = layers.gather_nd(
            input=graph_attn_mask, index=pos_down_ind)
        ...
        graph_attn_mask_select = graph_attn_mask_up * (1.0 - (layers.cast(pos_up, dtype='float32') - pos)) + \
            graph_attn_mask_down * \
            (1.0 - (pos - layers.cast(pos_down, dtype='float32')))
    ...
    def graph_scaled_dot_product_attention(q, k, v, attn_bias, d_key, dropout_rate,
        ...
        # (batch_size, n_heads, tgt_len, n_block)
        product = layers.matmul(x=scaled_q, y=k, transpose_y=True)
        if attn_bias:
        ...
        if graph_attn_bias:
            # re-weight the attention score with gaussian weights
            gaussian_w = __compute_graph_bias(
                scaled_q, graph_attn_bias, pos_win)
        ...
        # [batch, n_heads, query_len, dim_per_hed]
        out = layers.matmul(weights, v)

        # Project back to the model size.
        # (batch_size, query_len, emb_dim)
        combine_out = __combine_heads_sent(out)
    ...
    # (batch_size, n_heads, query_len, dim_per_head)
    q_s, k_s, v_s = __split_heads_qkv_sent(
        q_s, k_s, v_s, n_head, d_key, d_value)

    #print("Global Graph Attention xj shape: %s" % (str(v_s.shape)))
    ...
    def scaled_dot_product_attention_with_sen_norm(q, k, v, attn_bias, d_key, dropout_rate, attn_s):
        ...
        :return:
        """
        #print("Global Local Attention xij shape: %s" % (str(v.shape)))
        # print("q.shape = %s" % str(q.shape))
        # (batch_size, n_block, n_head, tgt_len, dim_per_head)
        q = layers.expand(layers.unsqueeze(
            q, axes=[1]), expand_times=[1, key_s_len, 1, 1, 1])
        # print("q.shape = %s" % str(q.shape))
        # (batch_size*n_block, n_head, tgt_len, dim_per_head)
        ...
        # (batch_size, n_head, tgt_len, n_block, n_token)
        attn_w = layers.elementwise_mul(
            attn_w, layers.unsqueeze(attn_s, axes=[-1]), axis=0)
        # (batch_size, n_head, tgt_len, n_block*n_token)
        attn_w = layers.reshape(
            attn_w, shape=[batch_size, n_head, query_len, -1])
        ...
        values_w = layers.reshape(
            values_w, shape=[batch_size, n_head, -1, d_value])

        # (batch_size, n_head, tgt_len, dim_per_head)
        out = layers.matmul(attn_w, values_w)

        # Project back to the model size.
        # (batch_size, query_len, emb_dim)
        combine_out = __combine_heads_word(out)
    ...
    q_w, k_w, v_w = __split_heads_qkv_word(
        q_w, k_w, v_w, n_head, d_key, d_value)

    context_w, attn_w = scaled_dot_product_attention_with_sen_norm(q_w, k_w, v_w, attn_bias_w, d_key,
                                                                   dropout_rate, attn_s)

    # (batch_size, query_len, 2*emb_dim)
    combine_out = layers.concat([context_s, context_w], axis=2)
    # Project back to the model size.
