        ...
        while True:
            try:
                ...
                if steps % skip_steps == 0:
                    ...
                    logger.info("Step %d starting" % (steps))
                    train_exe.run(fetch_list=[])
                    logger.info("Step %d done" % (steps))
        ...
        try:
            ...
            if not do_dec:
                ...
                for seq_ids, seq_scores in zip(seq_ids_list, seq_scores_list):
                    ...
                    # print("Seq-ID: length: %d value: %s" %
                    #      (len(np.array(seq_ids).reshape(-1).tolist()), np.array(seq_ids).reshape(-1).tolist()))
                    # print("seq_scores-ID: length: %d value: %s" %
                    #      (len(np.array(seq_ids).reshape(-1).tolist()), np.array(seq_scores).reshape(-1).tolist()))
                    # print(seq_ids.lod())
