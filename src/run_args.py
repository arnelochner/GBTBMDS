...
model_g.add_arg("config_path", str, None,
                "Path to the json file for transformer seq2seq model config.")
...
run_type_g.add_arg("metrics", bool, True,
                   "Whether to perform evaluation on test data set.")
run_type_g.add_arg("stream_job", str, None,
                   "if not None, then stream finetuning task by job id.")
...
train_g.add_arg("learning_rate", float, 2.0,
...
train_g.add_arg("warmup_steps", int, 4000,
                "The training steps to perform linear learning rate warmup.")
...
train_g.add_arg("validation_steps", int, 10000,
                "The steps interval to evaluate model performance.")
train_g.add_arg("use_fp16", bool, False,
                "Whether to use fp16 mixed precision training.")
train_g.add_arg("use_dynamic_loss_scaling", bool, False,
...
train_g.add_arg("len_penalty", float, default=0.6,
                help="length penalty during decoding.")
train_g.add_arg("max_out_len", int, 300, "max length of decoding.")
train_g.add_arg("min_out_len", int, 200, "min length of decoding.")
train_g.add_arg("block_trigram", bool, True,
                "remove repeated trigrams in summary.")
...
data_g.add_arg("max_tgt_len", int, 300,
               "the number of tokens in target summaries.")
data_g.add_arg("max_examples_per_file", int, None,
               "Number of examples use per json file.")
data_g.add_arg("batch_size", int, 4096,
...
transformer_g.add_arg("max_seq_len", int, 512,
...
graphsum_g.add_arg("max_para_len", int, 60,
                   "Number of words in longest paragraph.")
graphsum_g.add_arg("max_para_num", int, 30,
                   "Number of paragraphs of the longest documents.")
...
roberta_graphsum_g.add_arg("roberta_vocab_file", str,
                           'roberta_config/vocab.txt', "roberta vocab")
roberta_graphsum_g.add_arg("encoder_json_file", str,
                           'roberta_config/encoder.json', 'bpt map')
roberta_graphsum_g.add_arg("vocab_bpe_file", str,
                           'roberta_config/vocab.bpe', "vocab bpe")
roberta_graphsum_g.add_arg(
    "use_interval", bool, False, "whether model interval")
...
ernie_graphsum_g.add_arg("ernie_vocab_file", str,
                         'ernie_config/vocab.txt', "ernie vocab")
